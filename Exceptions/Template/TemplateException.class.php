<?php
/**
 * @name TemplateException.class.php Gestionnaire d'exception spécifique relatives aux templates.
 * @author web-Projet.com (jean-luc.aubert@web-projet.com) - Juin 2016
 * @package wp\Exceptions\Template
 * @version 0.1.0
**/
namespace wp\Exceptions\Template;

use \wp\Exceptions\wpException;
use \wp\Exceptions\Errors\error;
use \wp\Utilities\Logger\Logger as Logger;

class TemplateException extends \wp\Exceptions\wpException{
	...
	protected function process(Error $error){
		...
		if($error->doLog()){
			$logger = new Logger("templateException");
